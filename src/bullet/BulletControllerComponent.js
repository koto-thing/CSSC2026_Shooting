...
export class BulletControllerComponent extends Component {
   ...
   */
  constructor({ boundsProvider, moveSpeed = 600, direction = Vector2.up() } = {}) {
    super();
  ...
  tick(deltaTime) {
    ...
    const pattern = BulletMovePatterns[this.movePattern] ?? BulletMovePatterns.straight;
  ...
  #isOutsideBounds() {
    ...
    // 弾が境界の外に出た場合はtrueを返す
    return (
      this.transform.x < bounds.x - margin ||
