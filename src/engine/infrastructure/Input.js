...
export class Input {
  ...
  static initialize(canvas) {
    ...
    canvas.addEventListener("contextmenu", Input.#handlers.contextMenu);
  ...
  static tick() {
    if (!Input.#initialized) {
      throw new Error("Input.initialize()を先に呼び出してください。");
    }
  ...
  static dispose() {
    ...
    window.removeEventListener("keydown", Input.#handlers.keyDown);

    window.removeEventListener("keyup", Input.#handlers.keyUp);

    window.removeEventListener("mouseup", Input.#handlers.mouseUp);

    window.removeEventListener("blur", Input.#handlers.blur);

    Input.#canvas.removeEventListener("mousedown", Input.#handlers.mouseDown);

    Input.#canvas.removeEventListener("mousemove", Input.#handlers.mouseMove);

    Input.#canvas.removeEventListener("wheel", Input.#handlers.wheel);

    Input.#canvas.removeEventListener("contextmenu", Input.#handlers.contextMenu);
  ...
  static #updateMousePosition(event) {
    ...
    Input.#mousePosition.x = (event.clientX - rect.left) * scaleX;

    Input.#mousePosition.y = (event.clientY - rect.top) * scaleY;
  }
